...
def dumb_but_optimized_inverse_kinematics(x, y, z):
    ...
    theta0_grid, theta1_grid, theta2_grid = np.meshgrid(
        theta0_range, theta1_range, theta2_range, indexing="ij"
    )
    ...
    return (
        best_theta0 * 180 / np.pi,
        best_theta1 * 180 / np.pi,
        best_theta2 * 180 / np.pi,
    )
