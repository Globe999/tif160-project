...
while True:
    ...
    for key in lower.keys():
        ...
        mask = cv2.dilate(mask, np.ones((3, 3), np.uint8), iterations=1)  # Dilate the mask

        combined_mask = cv2.bitwise_or(combined_mask, mask)  # Combine masks of all colors
    ...
    for i, cnt in enumerate(clist):
        approx = cv2.approxPolyDP(cnt, 0.02 * cv2.arcLength(cnt, True), True)  # Approximate the contour shape

        # Draw the contour on the frame
        cv2.drawContours(frame, [cnt], -1, colors[ks[i]], 2)  # Draw the contour with the color corresponding to the detected shape
        ...
        # Square
        if len(approx) == 4:
            x2, y2 = approx.ravel()[2], approx.ravel()[3]  # x2, y2 coordinates from approx
            x4, y4 = approx.ravel()[6], approx.ravel()[7]  # x4, y4 coordinates from approx
            side1 = abs(x2 - x)
            side2 = abs(y4 - y)
        ...
        # Hexagon
            ...
            if circularity < 0.8 and aspect_ratio > 0.8:  # Prevent confusion with circles 
                cv2.putText(frame, ks[i] + " Hexagon", (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, colors[ks[i]], 2)

        # Star
        elif len(approx) == 12:  # 12 sides
            cv2.putText(frame, ks[i] + " Star", (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, colors[ks[i]], 2)
